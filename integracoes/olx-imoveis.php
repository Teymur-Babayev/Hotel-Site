...
<Carga xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" 
    ...
    <Imoveis>
        <?php
			$_sql = mysqli_query($link, "SELECT * FROM tb_imoveis_anuncios WHERE status='S' AND integracao_olx_imoveis='S'");
        ...
        <Imovel>
			...
			?>
            <CodigoImovel><?php echo strtoupper(utf8_encode($_dados['referencia']));?></CodigoImovel>
            <?php
				}
			?>
            <?php
			...
			?>
            <TituloAnuncio><?php echo ucwords(utf8_encode($_dados['nome']));?></TituloAnuncio>
            <?php
				}
			?>
            <?php
            ...
            <SubTipoImovel><?php 				
					...
					switch ($_dados['id_tipo']) {
						...
						case  2: echo "Galpão / Depósito / Barracão"; break;
			...
			?>
            <AreaTotal><?php echo $_dados['tamanho_area_total'];?></AreaTotal>
            <?php
				}
			?>
            <?php
			...
			?>
            <AreaUtil><?php echo $_dados['tamanho_area_privativa'];?></AreaUtil>
            <?php
				}
			?>
            <?php
